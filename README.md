...
- The loader writes no log by default. Create an empty file at `ux0:data/Bully/log` and it will write one to `ux0:data/bully_log.txt` -- a line every twenty seconds saying what the texture cache is holding and what it has had to do to hold it. That is what to attach to a bug report.
